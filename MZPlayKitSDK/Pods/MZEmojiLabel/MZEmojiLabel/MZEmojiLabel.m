...
#import "MZEmojiLabel.h"

#pragma mark - 正则列表

#define MZ_REGULAREXPRESSION_OPTION(regularExpression,regex,option) \
\
...
#define MZ_REGULAREXPRESSION(regularExpression,regex) MZ_REGULAREXPRESSION_OPTION(regularExpression,regex,NSRegularExpressionCaseInsensitive)


MZ_REGULAREXPRESSION(MZ_URLRegularExpression,@"((http[s]{0,1}|ftp)://[a-zA-Z0-9\\.\\-]+\\.([a-zA-Z]{2,4})(:\\d+)?(/[a-zA-Z0-9\\.\\-~!@#$%^&*+?:_/=<>]*)?)|(www.[a-zA-Z0-9\\.\\-]+\\.([a-zA-Z]{2,4})(:\\d+)?(/[a-zA-Z0-9\\.\\-~!@#$%^&*+?:_/=<>]*)?)|(((http[s]{0,1}|ftp)://|)((?:(?:25[0-5]|2[0-4]\\d|((1\\d{2})|([1-9]?\\d)))\\.){3}(?:25[0-5]|2[0-4]\\d|((1\\d{2})|([1-9]?\\d))))(:\\d+)?(/[a-zA-Z0-9\\.\\-~!@#$%^&*+?:_/=<>]*)?)")

MZ_REGULAREXPRESSION(MZ_PhoneNumerRegularExpression, @"\\d{3}-\\d{8}|\\d{3}-\\d{7}|\\d{4}-\\d{8}|\\d{4}-\\d{7}|1+[3578]+\\d{9}|\\d{8}|\\d{7}")

MZ_REGULAREXPRESSION(MZ_EmailRegularExpression, @"[A-Z0-9a-z\\._%+-]+@([A-Za-z0-9-]+\\.)+[A-Za-z]{2,4}")

MZ_REGULAREXPRESSION(MZ_AtRegularExpression, @"@[\\u4e00-\\u9fa5\\w\\-]+")

//@"#([^\\#|.]+)#"
MZ_REGULAREXPRESSION_OPTION(MZ_PoundSignRegularExpression, @"#([\\u4e00-\\u9fa5\\w\\-]+)#", NSRegularExpressionCaseInsensitive)
...
//MZ_REGULAREXPRESSION(MZ_SlashEmojiRegularExpression, @"/:[\\x21-\\x2E\\x30-\\x7E]{1,8}")
 ...
 */
@interface MZEmojiLabelRegexPlistManager : NSObject
...
@implementation MZEmojiLabelRegexPlistManager

+ (instancetype)sharedInstance {
    static MZEmojiLabelRegexPlistManager *_sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
...
@interface TTTAttributedLabel(MZEmojiLabel)
...
@interface MZEmojiLabel()

@property (nonatomic, weak) NSRegularExpression *customEmojiRegularExpression;
@property (nonatomic, weak) NSDictionary *customEmojiDictionary; //这玩意如果有也是在MZEmojiLabelPlistManager单例里面存着
...
- (NSMutableAttributedString*)mutableAttributeStringWithEmojiText:(NSAttributedString *)emojiText
{
    ...
    if (self.customEmojiRegularExpression) {
        ...
        emojis = [kMZ_SlashEmojiRegularExpression() matchesInString:emojiText.string
                                                         options:NSMatchingWithTransparentBounds
                                                           range:NSMakeRange(0, [emojiText length])];
    }
    ...
    for (NSTextCheckingResult *result in emojis) {
        ...
        NSDictionary *emojiDict = self.customEmojiRegularExpression?self.customEmojiDictionary:[MZEmojiLabel emojiDictionary];
        ...
        if (imageName) {
            ...
            NSMutableAttributedString *replaceStr = [[NSMutableAttributedString alloc] initWithString:kMZEmojiReplaceCharacter];
...
- (void)setText:(id)text
{
    ...
    NSRegularExpression * const regexps[] = {kMZ_URLRegularExpression(),kMZ_EmailRegularExpression(),kMZ_PhoneNumerRegularExpression(),kMZ_AtRegularExpression(),kMZ_PoundSignRegularExpression()};
    
    NSMutableArray *results = [NSMutableArray array];
    
    NSUInteger maxIndex = self.isNeedAtAndPoundSign?kMZURLActionCount:kMZURLActionCount-2;
...
- (void)setCustomEmojiRegex:(NSString *)customEmojiRegex
{
    ...
    if (customEmojiRegex&&customEmojiRegex.length>0) {
        self.customEmojiRegularExpression = [[MZEmojiLabelRegexPlistManager sharedInstance]regularExpressionForRegex:customEmojiRegex];
    }else{
...
- (void)setCustomEmojiPlistName:(NSString *)customEmojiPlistName
{
    ...
    if (customEmojiPlistName&&customEmojiPlistName.length>0) {
        self.customEmojiDictionary = [[MZEmojiLabelRegexPlistManager sharedInstance]emojiDictForKey:customEmojiPlistName];
    }else{
...
{
    //如果delegate实现了mzEmojiLabel自身的选择link方法
    if(self.delegate&&[self.delegate respondsToSelector:@selector(mzEmojiLabel:didSelectLink:withType:)]){
