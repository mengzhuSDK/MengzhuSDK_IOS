...
-(void)updateSubviewsWithModel:(MZLiveFinishModel *)model{
    self.nameLabel.text = [MZBaseUserServer currentUser].nickName;
    
    [self.avatarImageView sd_setImageWithURL:[NSURL URLWithString:[MZBaseUserServer currentUser].avatar] placeholderImage:[UIImage imageNamed:@"liveFinish_avator"]];
    self.UIDLabel.text =[NSString stringWithFormat:@"uniqueId:%@",[MZBaseUserServer currentUser].uniqueID] ;
