...
-(void)getLiveData
{
    NSString *codeName = [MZBaseUserServer currentUser].nickName;
    NSString *codeAvatar = [MZBaseUserServer currentUser].avatar;

    [MZSDKBusinessManager startLiveWithUsername:codeName userAvatar:codeAvatar uniqueId:[MZBaseUserServer currentUser].uniqueID ticketId:self.ticket_id live_tk:self.live_tk success:^(id response) {
