...
- (void)layoutSubviews {
    ...
    if (UIScreen.mainScreen.bounds.size.width > UIScreen.mainScreen.bounds.size.height) {
        ...
        self.redPackageButton.frame = CGRectMake(self.headerBtn.right + 5*self.space, self.nickNameL.bottom + 8*self.space, 240*self.space, 71*self.space);
...
-(void)setPollingDate:(MZLongPollDataModel *)pollingDate{
    ...
    self.redPackageButton.frame = CGRectMake(_iconHeight + 16*self.space + 5*self.space, self.nickNameL.bottom + 8*self.space, 240*self.space, 71*self.space);
...
+ (float)getCellHeightIsLand:(BOOL)isLand {
    ...
    return  70*space + 8*space + 18 + 16*space;
}
