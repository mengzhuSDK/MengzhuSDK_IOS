...
typedef void (^MZAnimationCompletionBlock)(BOOL animationFinished);

@interface MZAnimationView : MZView
...
/// Creates an animation from the MZComposition, images are loaded from the specified bundle
- (nonnull instancetype)initWithModel:(nullable MZComposition *)model inBundle:(nullable NSBundle *)bundle;
...
/// Load animation by name from the default bundle. Use when loading MZAnimationView via Interface Builder.
...
@property (nonatomic, copy, nullable) MZAnimationCompletionBlock completionBlock;

/// Set the animation data
@property (nonatomic, strong, nullable) MZComposition *sceneModel;
...
- (void)playToProgress:(CGFloat)toProgress
        withCompletion:(nullable MZAnimationCompletionBlock)completion;
 ...
 * If loopAnimation is NO the animation will stop and the completion block will be called.
 **/
- (void)playWithCompletion:(nullable MZAnimationCompletionBlock)completion;
...
/*!
 @brief Sets a MZValueDelegate for each animation property returned from the MZKeypath search. MZKeypath matches views inside of MZAnimationView to their After Effects counterparts. The MZValueDelegate is called every frame as the animation plays to override animation values. A delegate can be any object that conforms to the MZValueDelegate protocol, or one of the prebuilt delegate classes found in MZBlockCallback, MZInterpolatorCallback, and MZValueCallback.

 @discussion
 Example that sets an animated stroke to Red using a MZColorValueCallback.
 @code
 MZKeypath *keypath = [MZKeypath keypathWithKeys:@"Layer 1", @"Ellipse 1", @"Stroke 1", @"Color", nil];
 MZColorValueCallback *colorCallback = [MZColorBlockCallback withColor:[UIColor redColor]];
 [animationView setValueDelegate:colorCallback forKeypath:keypath];
 @endcode

 See the documentation for MZValueDelegate to see how to create MZValueCallbacks. A delegate can be any object that conforms to the MZValueDelegate protocol, or one of the prebuilt delegate classes found in MZBlockCallback, MZInterpolatorCallback, and MZValueCallback.

 See the documentation for MZKeypath to learn more about how to create keypaths.
 ...
 */
- (void)setValueDelegate:(id<MZValueDelegate> _Nonnull)delegates
              forKeypath:(MZKeypath * _Nonnull)keypath;

/*!
 @brief returns the string representation of every keypath matching the MZKeypath search.
 */
- (nullable NSArray *)keysForKeyPath:(nonnull MZKeypath *)keypath;
...
- (CGPoint)convertPoint:(CGPoint)point
         toKeypathLayer:(nonnull MZKeypath *)keypath;
 ...
 */
- (CGPoint)convertPoint:(CGPoint)point
       fromKeypathLayer:(nonnull MZKeypath *)keypath;
 ...
 */
- (void)addSubview:(nonnull MZView *)view
    toKeypathLayer:(nonnull MZKeypath *)keypath;
 ...
 */
- (void)maskSubview:(nonnull MZView *)view
     toKeypathLayer:(nonnull MZKeypath *)keypath;

#if !TARGET_OS_IPHONE && !TARGET_OS_SIMULATOR
@property (nonatomic) MZViewContentMode contentMode;
#endif
...
/*!
 @brief Converts the given CGRect from the receiving animation view's coordinate space to the supplied layer's coordinate space If layerName is null then the rect will be converted to the composition coordinate system. This is helpful when adding custom subviews to a MZAnimationView
