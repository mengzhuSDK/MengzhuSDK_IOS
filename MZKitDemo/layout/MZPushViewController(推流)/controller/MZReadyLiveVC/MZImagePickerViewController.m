...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info
{
    ...
    if (portraitImg.size.width/(portraitImg.size.height*1.0) != 1) {
        portraitImg = [MZGlobalTools imageByScalingToMaxSize:portraitImg size:CGSizeMake(1920, 1920)];//缩小
        portraitImg = [MZGlobalTools imageByScalingToMinSize:portraitImg size:CGSizeMake(360, 360)];//放大
    }
    if(self.ratio <= 0){
        self.ratio = 1;
    }
...
- (void)imageCropper:(VPImageCropperViewController *)cropperViewController didFinished:(UIImage *)editedImage {
    ...
    if (editedImage !=nil)
//        self.imageBlock([MZGlobalTools scaleToSize:editedImage size:CGSizeMake(120, 120)]);
